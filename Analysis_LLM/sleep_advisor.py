...
def analyze_sleep_data(sensor_summary):
    ...
    [측정 데이터]
    - 뒤척임 비율: {sensor_summary.get('avg_movement', 0):.1f}% (0단계는 숙면,3단계는 많은 움직임, 4단계는 깨어났다고 판단되는 정도)
    ...
    3. 뒤척임 단계가 낮을수록 높은 점수를 부여하세요. (0: 움직임이 없는 이상적인 수면,1: 경미한 움직임을 보이는 수면의 단계,2: 수면의 질이 낮다고 판단되는 수면의 단계,3: 수면 장애가 있다고 판단되는 수면의 단계  ,4: 외부적인 요소, 수면상태가 아니라고 판단되는 예외의 단계)
